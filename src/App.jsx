...
export default function App() {
  ...
  const [pages, setPages] = useState(
    Array.isArray(persisted.pages) ? persisted.pages : DEFAULT_PAGES,
  )
  ...
  const normalizedHref =
    normalizedSlug === 'top' || normalizedSlug === 'home'
      ? '/'
      : `/${normalizedSlug}/`
  const isDuplicateSlug =
    normalizedSlug !== '' &&
    pages.some((p) => p.slug === normalizedSlug || p.href === normalizedHref)
  const canAdd =
    nameField.trim() !== '' && normalizedSlug !== '' && !isDuplicateSlug
  ...
  const addPage = useCallback(() => {
    ...
    setPages((prev) => [
      ...prev,
      { name, slug: normalizedSlug, href: normalizedHref },
    ])
    setNameField('')
    setSlugField('')
